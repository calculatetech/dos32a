/****************************************************************************
*  A DLL test program for the DOS4G API using Watcom
*
 ...
 */	 
   ...
   if (argc<3) 
	...
	if (procHandle) // use the module via the handle returned by D32LoadModule()
      ...
      // Note that the ordinals are ignored when a function name is passed in
	   fp =  D32QueryProcAddr((int)procHandle,(DWORD) 0, (char *)fName,
		...
		// If building with the -#r switch enabled, for register based passing, 
      // "UTLmax" below must be replaced with "UTLmax_"
		strcpy(fName, "UTLmax");
   	fp =  D32QueryProcAddr((int)procHandle,(DWORD) 1, (char *) fName,
			(DWORD *) &fp);
		if (fp) 
   		{
			printf("D32QueryProcAddr(%d,0,%s) returned %#X\n",
		...
		// If building with the -#r switch enabled, for register based passing, 
      // "UTLmin" below must be replaced with "UTLmin_"
		strcpy(fName, "UTLmin");
	   fp =  D32QueryProcAddr((int)procHandle,(DWORD) 1, (char *) fName,
			(DWORD *) &fp);
		if (fp) 
	...
	// Since DLL functions loaded implicitly and any register based compilation
   // concerns handled in mydll.def just call away
